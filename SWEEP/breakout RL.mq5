...
//------------------------------------------------------------------
// Haal kale ATR-waarde (in prijsunits) via indicator-handle
//------------------------------------------------------------------
double GetAtrValue()
  {
...
void UpdateStats(int state_id, int action_id, double reward)
  {
   ...
   g_stats[state_id][action_id].total_reward += reward;
   g_stats[state_id][action_id].count++;
   g_stats[state_id][action_id].avg_reward =
      g_stats[state_id][action_id].total_reward /
      (double)g_stats[state_id][action_id].count;
  }
...
double CalculateVolume(double sl_price, double entry_price)
  {
   ...
   // Bepaal volume-digits uit de stapgrootte
   int digits = 0;
   if(step > 0.0)
     {
      double tmp = step;
...
void ManageTrailing()
  {
   ...
   if(!SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL, stop_level_raw))
      stop_level_raw = 0;

   double stop_level_points = (double)stop_level_raw;
...
//------------------------------------------------------------------
// OnInit: initialiseer profielen en laad RL-stats + ATR handle
//------------------------------------------------------------------
int OnInit()
  {
   ...
   g_atr_handle = iATR(_Symbol, AtrTF, ATR_Period);
   if(g_atr_handle == INVALID_HANDLE)
     {
      Print("Kan ATR-indicator niet initialiseren: ", _LastError);
      return INIT_FAILED;
     }
...
//------------------------------------------------------------------
// OnDeinit: stats saven + ATR handle releasen
//------------------------------------------------------------------
