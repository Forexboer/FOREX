...
//--- utility macros
#define  PIP()    (SymbolInfoDouble(_Symbol, SYMBOL_POINT) * ((_Digits==3 || _Digits==5)?10:1))
...
input double      MaxBoxRangePips      =100;       // Maximum allowed box range
...
input double      SL_FixedPips         =50;
input double      SL_BoxPercent        =50.0;
   ...
   if(InpFridayClose)
     {
      ...
      if(tm.day_of_week==5)
        {
         int hour=StringToInteger(StringSubstr(InpFridayCloseTime,0,2));
         int min=StringToInteger(StringSubstr(InpFridayCloseTime,3,2));
      ...
      if(TimeCurrent()>=st && TimeCurrent()<=en)
        {
         if(!sessions[i].box_ready)
           {
            if(sessions[i].high==0 || High[0]>sessions[i].high) sessions[i].high=High[0];
            if(sessions[i].low==0 || Low[0]<sessions[i].low) sessions[i].low=Low[0];
   ...
   for(int i=0;i<3;i++)
     {
      ...
      if((high-low)/PIP()>MaxBoxRangePips) continue; // skip large boxes
      ...
      if(UseDirectBreakout && TimeFilter(DirectStartTime,DirectEndTime))
        {
         if(!sessions[i].buy_done && Ask>high)
           {
            double sl=CalcStopLoss(true,high,low,Ask,StopLossMethod,SL_FixedPips);
         ...
         if(!sessions[i].sell_done && Bid<low)
           {
            double sl=CalcStopLoss(false,high,low,Bid,StopLossMethod,SL_FixedPips);
            double tp1=0,tp2=0,tp3=0;
      ...
      if(UseCandleClose && TimeFilter(CandleStartTime,CandleEndTime))
        {
         ...
         for(int c=1;c<=ConfirmCloses;c++)
           {
            double close=iClose(_Symbol,ConfirmTF,c-1);
   ...
   switch(sl_method)
     {
      ...
      case SL_BoxPercent:
         if(!UseCandleClose && ApplySLOnlyToCandle) sl = isBuy? boxLow : boxHigh;
           ...
           {
            double dist=boxsize*(SL_BoxPercent/100.0);
      ...
      for(int j=0;j<3;j++)
        {
         ...
         if(ObjectFind(0,name)>=0) tp_levels[j]=ObjectGetDouble(0,name,OBJPROP_PRICE1);
        }
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
     {
      ...
      if(!moved && BE_OnTP1)
        {
         string name=StringFormat("TP_%I64u_%d",PositionGetInteger(POSITION_TICKET),1);
         if(ObjectFind(0,name)>=0)
           {
            double tp=ObjectGetDouble(0,name,OBJPROP_PRICE1);
            if((isBuy && price>=tp) || (!isBuy && price<=tp)) moved=true;
           }
        }
      if(!moved && BE_OnTP2)
        {
         string name=StringFormat("TP_%I64u_%d",PositionGetInteger(POSITION_TICKET),2);
         if(ObjectFind(0,name)>=0)
           {
            double tp=ObjectGetDouble(0,name,OBJPROP_PRICE1);
            if((isBuy && price>=tp) || (!isBuy && price<=tp)) moved=true;
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
     {
      if(!PositionSelectByIndex(i)) continue;
      if(PositionGetInteger(POSITION_MAGIC)!=InpMagicNumber) continue;
      datetime open=PositionGetInteger(POSITION_TIME);
...
void DailyClosures()
  {
   ...
   int h=StringToInteger(StringSubstr(times[tm.day_of_week],0,2));
   int m=StringToInteger(StringSubstr(times[tm.day_of_week],3,2));
